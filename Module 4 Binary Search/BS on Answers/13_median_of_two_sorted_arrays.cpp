...
double findMedianSortedArraysOptimal(vector<int>& nums1, vector<int>& nums2) {
    ...
    int n2=nums2.size();
    int n=n1+n2;
    if(n1>n2) return findMedianSortedArraysOptimal(nums2,nums1);   //RUN BS only on the smaller array
...
int main(){
    ...
    vector<int> nums2={3,4,56,455,788};
    
    cout<<findMedianSortedArraysBrute(nums1,nums2)<<endl;
    cout<<findMedianSortedArraysBetter(nums1,nums2)<<endl;
